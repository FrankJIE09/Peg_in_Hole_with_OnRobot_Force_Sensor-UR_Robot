# 该文件定义了一个名为Assembly的类，该类包含了各种装配相关操作，包括力控、坐标系转换、姿态数据转换等。
# 具体实现了获取机器人状态、实时绘制力传感器数据、判断是否3点接触、判断是否装配成功、轴孔接触等功能。
# 其中，rotate为装配角度，x、y、z为装配位置，force_xyz为力控模式下的施力大小，velocity、acceleration为机器人的速度和加速度.
# Created by Jie Yu
import math
import random
import time
